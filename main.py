...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are CANCER, Tony Stark's AI assistant. "
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            ...
            "CANCER checks these topics once a day and alerts the user when there is a new development. "
    ...
    {
        "name": "shutdown_cancer",
        "description": (
            "Shuts down the assistant completely. "
            "Call this when the user expresses intent to end the conversation, "
            "close the assistant, say goodbye, or stop Cancer. "
...
class CancerLive:
    ...
    def __init__(self, ui: CancerUI):
        self.ui             = ui
        self._asst_name     = "CANCER"   # updated each session from config
        self.session              = None
    ...
    def plugin_say(self, instruction: str) -> None:
        """
        Thread-safe speech channel for plugins: lets a plugin ask CANCER to
    ...
    def interrupt(self) -> None:
        """Stop CANCER mid-speech: drain queued audio and open mic immediately."""
        ...
        if q:
            ...
            while True:
                try:
                    ...
                    break
            if drained:
                print(f"[CANCER] ✋ Interrupted — {drained} audio chunks discarded")
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        try:
            ...
            self._asst_name = (_cfg.get("assistant_name") or "CANCER").strip()
            _user_name = (_cfg.get("user_name") or "").strip()
        except Exception:
            self._asst_name = "CANCER"
            _user_name = ""
        ...
        cfg = dict(
            ...
            # window — CANCER can stay in one conversation for hours
        ...
        if self._enhanced_live:
            # Affective dialog: CANCER hears tone/emotion and adapts its voice.
            # Proactive audio: CANCER stays silent when speech isn't addressed
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        if name == "save_memory":
            ...
            if key and value:
                ...
                print(f"[Memory] 💾 save_memory: {category}/{key} = {value} ({today_str})")
        ...
        print(f"[CANCER] 📤 {name} → {str(result)[:80]}")
        return types.FunctionResponse(
            id=fc.id, name=name,
    ...
    async def _listen_audio(self):
        print("[CANCER] 🎤 Mic started")
        loop = asyncio.get_event_loop()

        def callback(indata, frames, time_info, status):
            with self._speaking_lock:
                cancer_speaking = self._is_speaking
            if not cancer_speaking and not self.ui.muted and not self._phone_active:
                data = indata.tobytes()
        ...
        try:
            with sd.InputStream(
                ...
                print("[CANCER] 🎤 Mic stream open")
                while True:
                    await asyncio.sleep(0.1)
        except Exception as e:
            print(f"[CANCER] ❌ Mic: {e}")
            raise

    async def _receive_audio(self):
        print("[CANCER] 👂 Recv started")
        out_buf, in_buf = [], []
        ...
        try:
                        ...
                        if sc.turn_complete:
                            ...
                            if self._pending_vision and self.session:
                                ...
                                if self._vision_cam_active:
                                    # Camera: keep busy until CANCER finishes speaking the answer
            ...
            print(f"[CANCER] ❌ Recv: {e}")
            traceback.print_exc()
            raise

    async def _play_audio(self):
        print("[CANCER] 🔊 Play started")
    ...
    async def _run_background_monitor(self) -> None:
        ...
        while True:
            if self.session:
                # Don't interrupt if user spoke recently or CANCER is mid-sentence
                with self._speaking_lock:
                    speaking = self._is_speaking
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    print("[CANCER] Connected.")
                    self.ui.set_state("LISTENING")
                    self.ui.write_log("SYS: CANCER online.")
                ...
                err_str = str(e)
                print(f"[CANCER] Error ({type(e).__name__}): {e}")
                traceback.print_exc()
                ...
                if "API key not valid" in err_str or "1007" in err_str:
                    ...
                    print("[CANCER] New API key saved — reconnecting...")
                    _conn_backoff = 3
                    continue
            ...
            print(f"[CANCER] Reconnecting in {delay}s...")
            await asyncio.sleep(delay)

def main():
    ui = CancerUI("face.png")

    def runner():
        ui.wait_for_api_key()
        cancer = CancerLive(ui)
        try:
            asyncio.run(cancer.run())
        except KeyboardInterrupt:
            print("\n🔴 Shutting down...")
