"""
dashboard/server.py — CANCER Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "CANCER Uploads",
        Path.home() / "Documents" / "CANCER Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
# ── AES-256-CBC ───────────────────────────────────────────────────────────────
_AES_SALT = b'CANCER-DASHBOARD-v1'
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"CANCER Dashboard Port {port}"
        prog_rule  = "CANCER Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="cancer_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until CANCER is run as Administrator.")
...
class DashboardServer:
    ...
    def _ssl_enabled() -> bool:
        ...
        return (certs / "cancer.key").exists() and (certs / "cancer.crt").exists()
...
<p>Press <strong style="color:#dde3ed">Remote Control</strong> in CANCER to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('cancer_token','{tok}');
  sessionStorage.setItem('cancer_key','{key}');
  localStorage.setItem('cancer_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to CANCER…</p>
</body></html>""")
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "cancer.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "cancer.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Remote Control' in CANCER UI to get the QR code.")
        await uvicorn.Server(cfg).serve()