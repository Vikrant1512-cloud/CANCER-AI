...
from PyQt6.QtGui import (
    ...
    QFontDatabase, QIcon, QKeySequence, QLinearGradient, QMouseEvent,
...
class HudArcCanvas(QWidget):
    def __init__(self, face_path: str, assistant_name: str = "CANCER", parent=None):
        ...
        self._scan       = 0.0
        self._scan2      = 180.0
        self._rings      = [0.0, 120.0, 240.0, 45.0]
        self._pulses: list[float] = [0.0, 50.0, 100.0]
        self._blink      = True
        self._blink_tick = 0
        self._particles: list[list[float]] = []
        self._wave_history: list[float] = [0.0] * 72
    ...
    def _load_face(self, path: str):
        candidates = [
            Path(path),
            CONFIG_DIR / "cancer.ico",
            BASE_DIR / "cancer.ico",
            CONFIG_DIR / "cancer.png",
            BASE_DIR / "cancer.png",
            CONFIG_DIR / "jarvis.ico"
        ]
        for p in candidates:
            if p.exists():
                try:
    ...
    def _step(self):
        ...
        self._float_y = math.sin(self._tick * 0.07) * 4.5
        ...
        speeds = [1.3, -1.5, 0.9, -0.7] if self.speaking else [0.55, -0.65, 0.4, -0.3]
        for i, spd in enumerate(speeds):
            self._rings[i] = (self._rings[i] + spd) % 360

        self._scan  = (self._scan  + (3.2 if self.speaking else 1.3)) % 360
        self._scan2 = (self._scan2 + (-2.2 if self.speaking else -0.75)) % 360

        fw  = min(self.width(), self.height())
        lim = fw * 0.74
        pspd = 4.2 if self.speaking else 2.0
        self._pulses = [r + pspd for r in self._pulses if r + pspd < lim]
        if len(self._pulses) < 3 and random.random() < (0.07 if self.speaking else 0.025):
            self._pulses.append(0.0)

        if self.speaking and random.random() < 0.28:
            cx, cy = self.width() / 2, (self.height() / 2 - 20) + self._float_y
        ...
        new_val = random.uniform(8, 42) if self.speaking else math.sin(self._tick * 0.12) * 6 + 9
        self._wave_history.pop(0)
        self._wave_history.append(new_val)
    ...
    def paintEvent(self, _):
        ...
        cx, cy = W / 2, (H / 2 - 20) + self._float_y
        fw = min(W, H)
        ...
        tech_rings = [
            (fw * 0.44, 3.5, self._rings[0], 160, C.PRI),
            (fw * 0.41, 2.5, self._rings[1], 210, C.ACC),
            (fw * 0.38, 2.0, self._rings[2], 140, C.GREEN),
            (fw * 0.35, 3.0, self._rings[3], 180, "#9d00ff"),
            (fw * 0.31, 1.5, -self._rings[0], 260, C.PRI),
        ]

        for r, w, start_a, span_a, color in tech_rings:
            rect = QRectF(cx - r, cy - r, r * 2, r * 2)
            p.setPen(QPen(qcol(color, 220), w, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap))
            p.setBrush(Qt.BrushStyle.NoBrush)
            p.drawArc(rect, int(start_a * 16), int(span_a * 16))

        r_ticks = fw * 0.47
        p.setPen(QPen(qcol(C.PRI, 90), 1))
        for deg in range(0, 360, 6):
            rad = math.radians(deg)
            tlen = 8 if deg % 30 == 0 else 3
            p.drawLine(
                QPointF(cx + (r_ticks - tlen) * math.cos(rad), cy - (r_ticks - tlen) * math.sin(rad)),
                QPointF(cx + r_ticks * math.cos(rad), cy - r_ticks * math.sin(rad))
            )
        ...
        if self._face_px:
            ...
            p.setFont(QFont("Consolas", int(fw * 0.085), QFont.Weight.Bold))
            now_str = time.strftime("%H:%M")
            p.drawText(QRectF(cx - 100, cy - 36, 200, 44), Qt.AlignmentFlag.AlignCenter, now_str)

            p.setFont(QFont("Consolas", 8, QFont.Weight.Bold))
            p.setPen(QPen(qcol(C.ACC2), 1))
            p.drawText(QRectF(cx - 70, cy + 10, 140, 18), Qt.AlignmentFlag.AlignCenter, f"[{self._assistant_name}]")

            p.setFont(QFont("Consolas", 7))
            stat_color = C.MUTED_C if self.muted else (C.GREEN if self.state == "LISTENING" else C.PRI)
            p.setPen(QPen(qcol(stat_color), 1))
            p.drawText(QRectF(cx - 80, cy + 28, 160, 18), Qt.AlignmentFlag.AlignCenter, f"● {self.state}")
        ...
        graph_w = min(W - 40, 560)
        gx0 = (W - graph_w) / 2
        gy0 = H - 20
        p.setPen(QPen(qcol(C.BORDER_A), 1))
        p.drawLine(QPointF(gx0, gy0), QPointF(gx0 + graph_w, gy0))

        step_x = graph_w / len(self._wave_history)
        path = QPainterPath()
        path.moveTo(gx0, gy0 - self._wave_history[0])
        for i, val in enumerate(self._wave_history):
            path.lineTo(gx0 + i * step_x, gy0 - val)
...
class MiniFloatingWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._pixmap = None
        for p in [CONFIG_DIR / "cancer.ico", BASE_DIR / "cancer.ico", CONFIG_DIR / "cancer.png", BASE_DIR / "cancer.png"]:
            if p.exists():
                px = QPixmap(str(p))
                if not px.isNull():
                    self._pixmap = px.scaled(68, 68, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
                    break
    ...
    def _step(self):
        self._tick += 1
        self._float_y = math.sin(self._tick * 0.08) * 4.5
        self.update()
    ...
    def paintEvent(self, _):
        ...
        cx, cy = self.width() / 2, (self.height() / 2 - 6) + self._float_y

        p.setPen(QPen(qcol(C.PRI, 190), 2))
        p.setBrush(QBrush(qcol(C.BG, 225)))
        p.drawEllipse(QRectF(cx - 44, cy - 44, 88, 88))

        if self._pixmap:
            p.drawPixmap(int(cx - 34), int(cy - 34), self._pixmap)
        else:
            p.setPen(QPen(qcol(C.PRI), 2))
            p.setFont(QFont("Segoe UI", 26, QFont.Weight.Bold))
            p.drawText(QRectF(cx - 32, cy - 32, 64, 64), Qt.AlignmentFlag.AlignCenter, "♋")

        p.setFont(QFont("Consolas", 7, QFont.Weight.Bold))
        p.setPen(QPen(qcol(C.PRI), 1))
        p.drawText(QRectF(0, self.height() - 16, self.width(), 14), Qt.AlignmentFlag.AlignCenter, "CANCER")
    ...
    def mouseMoveEvent(self, e: QMouseEvent):
        if self._is_dragging and (e.buttons() & Qt.MouseButton.LeftButton):
...
class LogWidget(QTextEdit):
    ...
    def _next(self):
        ...
        elif tl.startswith(_ai_pfx) or tl.startswith("cancer:"): self._tag = "ai"
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for CANCER", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        layout.addWidget(_lbl("Configure CANCER before first boot.", 9, color=C.PRI_DIM))
        layout.addSpacing(6)
...
class CustomizeOverlay(QWidget):
    ...
    def __init__(self, assistant_name="CANCER", user_name="",
        ...
        lay.addSpacing(4)
        lay.addWidget(_lbl("YOUR NAME  (leave blank for default greeting)", 8,
    ...
    def _save(self):
        name = self._name_input.text().strip() or "CANCER"
...
class RemoteKeyOverlay(QWidget):
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected — CANCER ready")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._assistant_name: str = (_cfg.get("assistant_name") or "CANCER").strip()
        ...
        self._mini_widget.restore_requested.connect(self._restore_from_mini)
        ...
        self.hud = HudArcCanvas(face_path, _display)
    ...
    def _minimize_to_corner(self):
        self.hide()
        self._mini_widget.move(self.pos())
        self._mini_widget.show()

    def _restore_from_mini(self):
        self._mini_widget.hide()
        self.move(self._mini_widget.pos())
        self.show()
    ...
    def _create_lnk_windows(lnk: str, target: str, args: str,
        ...
        try:
            ...
            sc.Description      = "CANCER AI Assistant"
    ...
    def _create_desktop_shortcut(self):
        ...
        ico_path = Path(__file__).resolve().parent / "config" / "cancer.ico"
        ...
        try:
            ...
            if _os == "Windows":
                ...
                lnk      = str(desktop / "CANCER.lnk")
                icon_loc = str(ico_path) if ico_path.exists() else f"{target},0"
                self._create_lnk_windows(lnk, target, str(script),
                                         str(script.parent), icon_loc)

            elif _os == "Darwin":
                app     = desktop / "CANCER.app"
                mac_dir = app / "Contents" / "MacOS"
                res_dir = app / "Contents" / "Resources"
                mac_dir.mkdir(parents=True, exist_ok=True)
                res_dir.mkdir(exist_ok=True)

                launcher = mac_dir / "CANCER"
                launcher.write_text(
                    "#!/usr/bin/env bash\n"
                    f'cd "{script.parent}"\n'
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '  <key>CFBundleExecutable</key><string>CANCER</string>\n'
                    '  <key>CFBundleIdentifier</key>'
                    '<string>com.cancer.assistant</string>\n'
                    '  <key>CFBundleName</key><string>CANCER</string>\n'
    ...
    def _build_header(self) -> QWidget:
        ...
        self._sub_lbl = QLabel("Autonomous Intelligence System")
        ...
        min_btn.clicked.connect(self._minimize_to_corner)
        lay.addWidget(min_btn)

        max_btn = QPushButton("🗖")
        max_btn.setFixedSize(26, 24)
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("CANCER AI Core", C.PRI_DIM))
        return w
    ...
    def _check_autostart(self) -> bool:
        try:
            if _OS == "Windows":
                ...
                try:
                    winreg.QueryValueEx(key, "CANCER_AI")
                    return True
                ...
                return (Path.home() / "Library" / "LaunchAgents"
                        / "com.cancer.assistant.plist").exists()
            else:
                return (Path.home() / ".config" / "autostart" / "cancer.desktop").exists()
        except Exception:
            return False
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                if currently_on:
                    winreg.DeleteValue(reg, "CANCER_AI")
                else:
                    pythonw = Path(sys.executable).parent / "pythonw.exe"
                    exe = str(pythonw if pythonw.exists() else sys.executable)
                    winreg.SetValueEx(reg, "CANCER_AI", 0, winreg.REG_SZ,
                                      f'"{exe}" "{script}"')
                winreg.CloseKey(reg)
            elif _OS == "Darwin":
                plist_dir = Path.home() / "Library" / "LaunchAgents"
                plist_dir.mkdir(parents=True, exist_ok=True)
                plist = plist_dir / "com.cancer.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.cancer.assistant</string>\n'
    ...
    def _open_customize(self):
        ...
        cw = self.centralWidget()
        ov = CustomizeOverlay(
            cfg.get("assistant_name", "CANCER") or "CANCER",
            cfg.get("user_name", ""),
    ...
    def _apply_name_update(self, name: str, user_name: str, ui_color: str = ""):
        self._assistant_name = name.strip() or "CANCER"
        display = self._assistant_name.upper()
        self.setWindowTitle(f"{display} — MARK LI")
        self._title_lbl.setText(display)
        self._sub_lbl.setText("Autonomous Intelligence System")
    ...
    def _on_setup_done(self, key: str, os_name: str):
        ...
        API_FILE.write_text(
            json.dumps({"gemini_api_key": key, "os_system": os_name, "assistant_name": "CANCER"}, indent=4),
            encoding="utf-8",
        )
        self._ready = True
        if self._overlay:
            ...
            self._overlay = None
        self._apply_state("LISTENING")
        self._assistant_name = _read_full_config().get("assistant_name", "CANCER") or "CANCER"
...
class CancerUI:
    def __init__(self, face_path: str = "config/cancer.ico", size=None):
