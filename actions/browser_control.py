...
def _real_profile_dir(browser: str) -> str:
    ...
    fallback = home / ".cancer_profiles" / browser
...
class _BrowserSession:
    ...
    async def _launch(self):
        ...
        if engine_name == "firefox":
            profile = _firefox_profile_dir() or str(
                Path.home() / ".cancer_profiles" / "firefox"
            )
            kwargs: dict = {
                "headless":    False,
                "slow_mo":     0,
            ...
            try:
                ...
                print(f"[Browser] Firefox real profile failed ({e}), using CANCER profile")
                cancer = str(Path.home() / ".cancer_profiles" / "firefox_cancer")
                Path(cancer).mkdir(parents=True, exist_ok=True)
                self._context = await engine_obj.launch_persistent_context(cancer, **kwargs)
        ...
        if engine_name == "webkit":
            safari_profile = str(Path.home() / ".cancer_profiles" / "safari")
        ...
        # CANCER otomasyon profiline geçilir — buraya bir kez giriş yapılan
        # hesaplar sonraki oturumlarda da açık kalır.
        cancer_profile = str(Path.home() / ".cancer_profiles" / self.browser_name)
        Path(cancer_profile).mkdir(parents=True, exist_ok=True)
        print(f"[Browser] Retrying with CANCER profile: {cancer_profile}")

        try:
            self._context = await engine_obj.launch_persistent_context(cancer_profile, **kwargs)
            self._page = await self._adopt_page()
            print(f"[Browser] ✅ Launched [{label}] with CANCER profile "
    ...
    async def screenshot(self, path: str = None) -> str:
        page = await self._get_page()
        try:
            save_path = path or str(Path.home() / "Desktop" / "cancer_screenshot.png")
